function CLF_CBF_RNDM_Disturbance_10m()
...
P.F_step = 0.40;
P.F_vert = 0.55;
P.a = 0.918; P.b = 0.372; P.c = 0.372; % Table 6.2 — semi-axes [m], visual only
...
% h1 at d=6.00m: 6.0^2 - 3.0^2 = 27.00
% Activation at 6m gives 3m stopping runway, guaranteeing inner barrier
% inviolability for CLF-driven approach speeds up to 3.36 m/s.
% Previous 4.5m activation only guaranteed up to 2.37 m/s — exceeded in
% testing at 80 Hz. Inner and outer activation now comparable in stopping
% authority relative to their respective expected approach velocities.
% h2 at d=5.00m: 10^2 - 5.0^2 = 75.00
P.cbf_h1_thresh        = 27.00;  % QP fires when d < 6.00m
...
% Simulation sample period. CBF barrier predictions (h_dot terms) use the
% state from the previous tick — smaller dt reduces the staleness window
% between uncontrolled dynamics (Coriolis, sway) and the QP's awareness.
% 0.025 s (40 Hz) is the baseline; 0.0125 s (80 Hz) for sharper turn fidelity.
% Timer Period and rk4step both use this value — change only here.
P.dt = 0.025;
...
%% FIGURE + AXES
%% ═══════════════════════════════════════════════════════════
fig = figure('Name','CLF and CBF Control Theory Validation at 10m', ...
...
tmr = timer('Name','FormationTimer','Period',P.dt, ...
...
function [inp, des_pos, ctrl_err] = autoFollower(S, L, P)
    ...
    % ── Outer zone: nearest cone-edge target [DISABLED for CBF validation] ──
    % Redirects CLF to nearest cone-edge point when B is outside ideal zone
    % and outside ±60° cone, avoiding path-through-A geometry. Commented out
    % so circumnavigation is accomplished by CLF/CBF interaction alone.
    % Re-enable for future guided approach applications.
...
%     end
    ...
    % ── Inner CBF circumnavigation [DISABLED for CBF validation] ────────────
    % Blends CLF target toward an orbit point when B is near the inner
    % boundary, cooperating with the outward CBF push. Commented out so
    % inner circumnavigation is driven by CLF/CBF alone without assistance.
    % Re-enable for future applications requiring explicit orbit guidance.
...
%     end
    ...
    % Arc correction [DISABLED for CBF validation] — tangential push back
    % into ±60° cone when B drifts outside it. Commented out so cone
    % realignment relies on CLF/CBF interaction alone.
    % Re-enable for future applications requiring explicit cone guidance.
    d_AB_arc = sqrt((S.x-L.x)^2 + (S.y-L.y)^2);
    arc_fx = 0; arc_fy = 0;  % disabled — set by arc correction when re-enabled
...
%     end
    ...
    psi_des        = psi_des_form;

    % ── Inner boundary tangential heading blend [DISABLED] ────────────
    % Functionally equivalent to arc correction — both add explicit
    % tangential guidance. Commented out for pure CLF+CBF validation.
%     d_AB_tang = sqrt((S.x-L.x)^2 + (S.y-L.y)^2);
%     h1_tang   = d_AB_tang^2 - P.cbf_d_min^2;
%     if h1_tang < P.cbf_h1_thresh
%         ang_AB_tang     = atan2(S.y - L.y, S.x - L.x);
%         ang_behind_tang = L.psi + pi;
%         d_ang_tang = atan2(sin(ang_AB_tang - ang_behind_tang),...
%                            cos(ang_AB_tang - ang_behind_tang));
%         psi_tang   = ang_AB_tang + sign(d_ang_tang)*pi/2 - beta;
%         tang_blend = max(0, min(1, h1_tang / P.cbf_h1_thresh));
%         dpsi_tang  = atan2(sin(psi_tang-psi_des), cos(psi_tang-psi_des));
%         psi_des    = psi_des + (1 - tang_blend) * dpsi_tang;
%     end
...
function [Fx_s, Fy_s, info] = clf_cbf_filter(Fx_nom, Fy_nom, S, L, des_xy, P, h3_cone)
    ...
    % Inner barrier — velocity-gated activation.
    % Activates when h1 < 0 (already violated) OR when B is within the
    % activation range AND actively approaching A (h1_dot < 0).
    % This prevents the inner CBF from disrupting normal formation
    % following at d=5.5m (h1_dot ≈ 0 there — no inward velocity, no
    % constraint). The full 6m activation range only fires when d is
    % actually closing, preserving stopping runway for high-speed
    % CLF-driven approaches without penalising steady-state tracking.
    inner_cbf_active = (h1 < 0) || (h1 < P.cbf_h1_thresh && h1_dot < 0);
    if inner_cbf_active
        rhs1 = 2*dv_sq ...
             - 2*(dx*aAx + dy*aAy) ...
             - 2*a_unc_dp ...
        ...
        rhs1 = 1e6;  % not approaching or outside zone — no constraint
    end
