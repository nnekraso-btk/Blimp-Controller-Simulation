...
% Comm zones
% Zone distances — 4m comm configuration (lab close-range scale).
P.comm_min_red    = 0.80;   % collision danger [m]
P.comm_min_yellow = 1.20;   % inner warning [m]
P.comm_ideal_lo   = 1.20;   % ideal zone lower bound [m]
P.comm_ideal_hi   = 2.50;   % ideal zone upper bound [m]
P.comm_max_yellow = 3.00;   % outer warning [m]
P.comm_max_red    = 4.00;   % comm loss boundary [m]
...
P.follow_dist = 1.60;         % desired separation [m]
...
P.cbf_d_min   = P.comm_min_red;     % [m] inner boundary = 0.80 m
P.cbf_d_max   = P.comm_max_red;     % [m] outer boundary = 4.00 m
% ECBF decay rates: lower = narrower pre-warning zone = less interference
P.cbf_alpha1  = 0.80;   % inner ECBF decay rate [1/s] — earlier, firmer response
P.cbf_alpha2  = 0.80;
...
% h1 at d=1.80m: 1.8^2 - 0.8^2 = 2.60
% h2 at d=2.00m: 4.0^2 - 2.0^2 = 12.00
P.cbf_h1_thresh        = 2.60;   % QP fires when d < 1.80m
P.cbf_h2_thresh        = 12.00;  % QP fires when d > 2.00m
P.cbf_broadside_thresh = 0.50;   % activate when within ~60° of broadside
P.cbf_broadside_d_min  = 2.00;   % [m] broadside check only when d > this
...
xlim(ax3,[-6 6]); ylim(ax3,[-6 6]); zlim(ax3,[-1 3]);
...
plot(axS,[-8 8],[-0.5 -0.5],'--','Color',[0.45 0.12 0.12],'LineWidth',0.8);
...
plot(axF,[-8 8],[0 0],'--','Color',[0.45 0.12 0.12],'LineWidth',0.8);
...
SB = fmState(-1.6, 0.2, 0, 0);  % Agent B near formation position
...
function [inp, des_pos, ctrl_err] = autoFollower(S, L, P)
    ...
    if isfield(S,'comm_lost') && S.comm_lost
        % Comm lost: navigate to last known leader position
    ...
    % Activates when B is past 75% of the cone limit (45°), proportional
    % to how far past the threshold B is. Zero inside the threshold.
    % This ensures B curves inward rather than taking the outside arc.
    cone_threshold = P.cone_warn_frac * P.cone_half_angle;
    arc_error = abs(d_angle) - cone_threshold;
    if arc_error > 0
        % Tangent at B's CURRENT position on the circle (not at behind-tail).
        % Using angle_behind here introduced a large radial outward component
        % = sin(d_angle), which at 60° was 0.87 — pushing B toward comm loss.
        % The correct tangent at angle_AB is purely tangential (zero radial).
    ...
    psi_des = atan2(ey, ex) - beta;                % crab-adjusted bearing
    ...
    % Rate of change of beta that Coriolis will produce next tick
    beta_dot  = v_dot_ff / max(0.05, abs(S.u));
    Mz = P.Kp_yaw*epsi - P.Kd_yaw*S.r - P.Kff_yaw*beta_dot;

    % Recovery Mz cap: when outside safe zone, limit yaw moment to preserve
    % minimum surge authority. Without this, large heading errors saturate both
    % motors on yaw and leave zero surge — the disturbance wins trivially.
    % Cap allows 45° turn in ~0.5s while guaranteeing >=60% surge authority.
    h2_ctrl   = P.cbf_d_max^2 - (S.x-L.x)^2 - (S.y-L.y)^2;
    if P.cbf_enabled && h2_ctrl < P.cbf_h2_thresh
        % Scale cap: full Mz inside zone, 0.691 N·m cap at boundary
        Mz_cap = (2*P.F_max - 0.60*P.F_max) * P.ly;  % preserves 60% surge
        h2_frac_mz = max(0, min(1, h2_ctrl / P.cbf_h2_thresh));
        Mz_limit = Mz_cap + h2_frac_mz * (10.0 - Mz_cap); % blend to large value inside
...
function [inp, SB] = applyDisturbance(inp, SB, SA, P, t)
    ...
    if is_active
        ...
        inp.dist_fx = P.dist_mag * (dx/d);
        inp.dist_fy = P.dist_mag * (dy/d);
        inp.dist_on = true;
    end
...
function drawFormation(H, SA, inpA, SB, inpB, P, E0, mode, xwpt, ywpt, des_pos, ctrl_err)
    ...
    % Span = comm_max_red + 5m margin on each side
    hw = P.comm_max_red + 2;  % half-width [m] — 2m margin beyond outer ring
    mz = SA.z;
    ...
    if SB.dist_on
        ...
        set(H.hDistLabel,'Position',[SB.x+SB.dist_fx*1.4, SB.y+SB.dist_fy*1.4],...
            'String',sprintf('DIST %.2fN',P.dist_mag),'Visible','on');
    else
...
function [Fx_s, Fy_s, info] = clf_cbf_filter(Fx_nom, Fy_nom, S, L, des_xy, P, h3_cone)
    ...
    % Inner barrier — hard, with worst-case inward disturbance margin
    rhs1 = 2*dv_sq ...
         - 2*(dx*aAx + dy*aAy) ...
         - 2*a_unc_dp ...
         - dist_margin_inner ...
...
function drawBlimp(hEnvS,hGndS,hDirS,hTrailS, ...
    ...
    % Display scale: sized for 4m comm / ±6m axis limits
    dr_f = 0.50;   % nose semi-axis [m display]
    dr_r = 0.65;   % tail semi-axis [m display]
    dr_w = 0.22;   % width semi-axis [m display]
